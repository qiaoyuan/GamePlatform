  ...
  methods: {
    ...
    initSwiperList(allDataList, contentIndex) {
      ...
      if (this.keyField === '_tid') {
        allDataList.forEach((item, index) => (item._tid = index + 1))
      }
    ...
    swiperChangeEvent(e) {
      ...
      const contentIndex = allDataList.findIndex(item => item?.[this.keyField] === currentItem?.[this.keyField])
      ...
      if (isForwardAdjacent || isLoopPositive) {
        ...
        this.renderList.splice(
          this.getNextVisibleIndex(visibleIndex),
          1,
          this.getNextRenderItem(allDataList, contentIndex)
        )
      }
      ...
      if (isReverseAdjacent || isLoopNegative) {
        ...
        // 使用 vue 官方包装的 splice 方法进行数据的增加，从而实现对列表数据更新后，渲染页面
        this.renderList.splice(
          this.getLastVisibleIndex(visibleIndex),
          1,
          this.getLastRenderItem(allDataList, contentIndex)
        )
      }
