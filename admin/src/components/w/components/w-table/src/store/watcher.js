...
export default Vue.extend({
  ...
  methods: {
    ...
    updateColumns() {
      ...
      if (fixedLeafColumns.length) {
        // 为固定列设置宽度
        fixedLeafColumns.reduce((left, { width, minWidth }, index) => {
          fixedLeafColumns[index].left = left
          return left + (width || minWidth)
        }, 0)
        // 为最后一个左固定列添加标识
        fixedLeafColumns[fixedLeafColumns.length - 1].isEdge = 'left'
      }

      const rightFixedLeafColumns = doFlattenColumns(states.rightFixedColumns)

      if (rightFixedLeafColumns.length) {
        // 为固定列设置宽度
        rightFixedLeafColumns.reduceRight((right, { width, minWidth }, index) => {
          rightFixedLeafColumns[index].right = right
          return right + (width || minWidth)
        }, 0)
        // 为第一个右固定列添加标识
        rightFixedLeafColumns[0].isEdge = 'right'
      }
