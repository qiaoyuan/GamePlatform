  ...
  >
    ...
    <div class="relative">
      ...
      <div
        ...
        @mousedown.self="clickTrackHandler($event, 'horizontal')"
      >
        <div
          ref="horizontalThumb"
          class="el-scrollbar__thumb"
          :style="{ width: horizontalLong + 'px' }"
      ...
      <div
        ...
        @mousedown.self="clickTrackHandler($event, 'vertical')"
      >
        <div
          ref="verticalThumb"
          class="el-scrollbar__thumb"
          :style="{ height: verticalLong + 'px' }"
...
const ScrollbarMap = {
  horizontal: { type: ['width', 'Width'], axle: ['left', 'right', 'Left', 'Right'], client: 'X' },
  vertical: { type: ['height', 'Height'], axle: ['top', 'bottom', 'Top', 'Bottom'], client: 'Y' },
}
...
export default {
  ...
  methods: {
    handleScroll() {
      const { bodyReallyWidth, bodyReallyHeight } = this.layout
      const { scrollLeft, scrollTop } = this.bodyWrapper
      const { horizontalTrack, horizontalThumb, verticalTrack, verticalThumb } = this.$refs
    ...
    clickTrackHandler(e, direction) {
      const { type, axle, client } = ScrollbarMap[direction]
    ...
    clickThumbHandler(e, direction) {
      ...
      this.thumbState = {
        trackLong, // 滚动槽长度
        thumbLong, // 滚动条长度
        thumbPosition: e['client' + client] - el.getBoundingClientRect()[axle[0]], // 鼠标点击滚动条的位置
    ...
    mouseMoveDocumentHandler: throttle(16, function (e) {
      ...
      const { trackLong, thumbPosition, maxOffset } = this.thumbState
      if (!thumbPosition || !maxOffset) return

      const { type, axle, client } = ScrollbarMap[this.direction]
      let offset =
        e['client' + client] -
        this.$refs[this.direction + 'Track'].getBoundingClientRect()[axle[0]] -
        thumbPosition
      if (offset < 0) {
        offset = 0
      } else if (offset >= maxOffset) {
      ...
      this.bodyWrapper['scroll' + axle[2]] =
        (offset / trackLong) * this.layout['bodyReally' + type[1]]
    }),
  ...
  computed: {
    ...
    emptyBlockStyle() {
      ...
      return { width: this.bodyWidth, height }
    },

    horizontalLong() {
      const { bodyShowWidth, bodyReallyWidth } = this.layout
      return (bodyShowWidth / bodyReallyWidth) * (bodyShowWidth - 4)
    },
    verticalLong() {
      const { bodyShowHeight, bodyReallyHeight } = this.layout
...
.el-scrollbar__bar {
  ...
  .el-scrollbar__thumb {
    ...
    background-color: rgba(61, 82, 102, 0.32);
    transition: background-color 150ms;
    &:hover {
      background-color: rgba(61, 82, 102, 0.5);
    }
  }
