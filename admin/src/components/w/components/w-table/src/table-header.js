...
export default {
            ...
            <tr style={this.getHeaderRowStyle(rowIndex)} class={this.getHeaderRowClass(rowIndex)}>
              {columns.map((column, cellIndex) => (
                <th
                  ...
                  on-dblclick={$event => this.handleHeaderDblclick($event, column)}
  ...
  methods: {
    ...
    getHeaderCellStyle(rowIndex, columnIndex, row, column) {
      ...
      if (this.tableLayout.scrollX) {
        ...
        if (column.fixed) {
          ...
          headerCellStyle.zIndex = '1'
        }
        // 补足滚动条宽度
        column.left !== undefined && (headerCellStyle.left = `${column.left}px`)
        column.right !== undefined && (headerCellStyle.right = `${column.right}px`)
      }
    ...
    getHeaderCellClass(rowIndex, columnIndex, row, column) {
      ...
      switch (column.fixed) {
        case 'left':
          classes.push('is-fixed-left-column')
          break
        case 'right':
          classes.push('is-fixed-right-column')
          break
      }
    ...
    // 计算所有直接子元素宽度（无法处理 padding margin flex 布局等样式导致的宽度影响）
    countWidth(children) {
      return Array.from(children).reduce(
        (pre, child) => pre + child.getBoundingClientRect().width,
        0
      )
    },
    // 递归计算所有子元素宽度（无法处理 padding margin flex 布局等样式导致的宽度影响）
    countWidthDeep(children) {
      return Array.from(children).reduce(
        (pre, child) =>
          pre + Math.max(child.getBoundingClientRect().width, this.countWidthDeep(child.children)),
        0
      )
    },
    // 更新列宽度
    setColumnWidth(column) {
      ...
      const th = this.$parent.$refs.tableHeader.$el.querySelector(`th.${column.id} > div.cell`)
      const tds = this.$parent.$refs.tableBody.$el.querySelectorAll(
        `td.${column.id} > div.el-tooltip`
      )
      const headerMaxWidth = this.countWidth(th.children)
      const contentMaxWidth = Array.from(tds).reduce(
        (pre, td) => Math.max(this.countWidthDeep(td.children), pre),
        0
      )

      column.minWidth = Math.ceil(Math.max(headerMaxWidth, contentMaxWidth)) + 6
      column.width = undefined
    },
    handleHeaderDblclick(event, column) {
      if (this.$isServer) return
      // 序号 / 操作 调整其他所有列
      if (['tableIndex', 'tableOperate'].includes(column.property)) {
        this.columns.forEach(
          column =>
            !['tableIndex', 'tableOperate'].includes(column.property) && this.setColumnWidth(column)
        )
      } else {
      ...
      setTimeout(this.$parent.doLayout, 0)
    },
