...
export default {
  ...
  computed: {
    ...
    timeFormat() {
      if (this.format) {
        return this.format.replace(this.dateFormat, '').trim()
    ...
    enableMonthArrow() {
      ...
      return (
        this.unlinkPanels &&
        new Date(this.leftYear + yearOffset, nextMonth) < new Date(this.rightYear, this.rightMonth)
      )
    },
      ...
      this.$nextTick(() => {
        if (this.$refs.maxTimePicker && this.maxDate && this.maxDate < this.minDate) {
          ...
          this.$refs.maxTimePicker.selectableRange = [
            [parseDate(formatDate(this.minDate, format), format), parseDate('23:59:59', format)],
          ]
        }
      })
  ...
  methods: {
    ...
    handleRangePick(val, close = true) {
      if (this.minDate && this.maxDate) {
        this.defaultTime = [
          dayjs(this.minDate).format('HH:mm:ss'),
          dayjs(this.maxDate).format('HH:mm:ss'),
        ]
