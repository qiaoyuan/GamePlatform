...
export default {
  ...
  methods: {
    cellMatchesDate(cell, date) {
      ...
      return (
        this.date.getFullYear() === value.getFullYear() && Number(cell.text) === value.getMonth()
      )
    },
    getCellStyle(cell) {
      const style = {}
    ...
    handleMonthTableClick(event) {
      ...
      if (this.selectionMode === 'range') {
        ...
        const year = this.date.getFullYear()
        const newValue =
          arrayFindIndex(value, date => date.getFullYear() === year && date.getMonth() === month) >=
          0
    ...
    rows() {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < 4; j++) {
          ...
          const time = new Date(this.date.getFullYear(), index).getTime()
          cell.inRange =
            time >= getMonthTimestamp(this.minDate) && time <= getMonthTimestamp(this.maxDate)
