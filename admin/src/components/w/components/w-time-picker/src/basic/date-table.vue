...
export default {
  ...
  computed: {
    ...
    rows() {
      ...
      const selectedDate =
        this.selectionMode === 'dates' ? coerceTruthyValueToArray(this.value) : []
      ...
      for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
          ...
          cell.inRange =
            time >= getDateTimestamp(this.minDate) && time <= getDateTimestamp(this.maxDate)
  ...
  methods: {
    ...
    getCellClasses(cell) {
      ...
      if (
        cell.inRange &&
        (cell.type === 'normal' || cell.type === 'today' || this.selectionMode === 'week')
      ) {
