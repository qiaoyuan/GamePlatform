<template>
  <div class="el-time-spinner" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      ...
      >
        <li
          @click="handleClick('hours', { value: hour })"
      ...
      >
        <li
          @click="handleClick('minutes', { value: minute })"
      ...
      >
        <li
          @click="handleClick('seconds', { value: second })"
          v-for="second in secondsList"
          class="el-time-spinner__item"
...
export default {
  ...
  computed: {
    ...
    minutesList() {
      ...
      return makeTimeList(
        hour === start ? this.start.minute() : 0,
        hour === end ? this.end.minute() : 60,
        this.step.m || this.step.minute,
        'minute'
      )
    },
    secondsList() {
      ...
      return makeTimeList(
        h === sh && m === sm ? this.start.second() : 0,
        h === eh && m === em ? this.end.second() : 0,
        this.step.s || this.step.second,
        'second'
      )
    },
    arrowHourList() {
    ...
    adjustSpinner(type, value) {
      ...
      if (el) {
        el.scrollTop = Math.max(
          0,
          this[`${type}List`].findIndex(v => v === value) * this.typeItemHeight(type)
        )
      }
    },
